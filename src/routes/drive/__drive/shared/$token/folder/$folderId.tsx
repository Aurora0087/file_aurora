...
export const Route = createFileRoute('/drive/__drive/shared/$token/folder/$folderId')({
  component: RouteComponent,
  errorComponent:FolderErrorComponent,
})

function RouteComponent() {
  const { token,folderId } = Route.useParams()
  ...
  // 3. Handle Navigation within the share
  ...
  if (status === 'LoadingFirstPage') {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm font-medium text-muted-foreground">Accessing shared items...</p>
      </div>
    )
  }
    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center gap-4 p-4 text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-xs">
          This link may have expired, been deleted, or you don't have permission to view it.
        </p>
      </div>
    )
  }
          ...
          item.type === 'folder' ? (
            <div key={item._id}>
              <FolderCard 
                ...
                token={token} // Pass token to allow navigation
              />
            </div>
          ) : (
            <FileCard 
